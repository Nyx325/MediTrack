...
GtkWidget *p_win, *p_grid, *p_entry[8], *p_lbl[9],  *p_combox[2], *p_btn[2], *p_checkbtn;
GdkPixbuf *p_icon;
...
void p_init_checkbtn(){
   p_checkbtn = gtk_check_button_new_with_label("Permiso de venta\nde narcóticos");
   g_signal_connect(p_checkbtn, "toggled", G_CALLBACK(on_check_button_toggled), NULL);
}
...
void p_set_widgets(){
    ...
    gtk_grid_attach(GTK_GRID(p_grid), p_entry[3], 3, 3, 1, 1);
